...
create table location (id integer not null, city varchar(255), country varchar(255), county varchar(255), name varchar(255), street_address varchar(255), primary key (id))
create table `order` (id integer not null, city varchar(255), country varchar(255), county varchar(255), street_address varchar(255), customer integer, shipped_from integer, primary key (id))
...
create table product (id integer not null, description varchar(255), name varchar(255), price decimal(19,2), weight double, category integer, supplier integer, primary key (id))
...
alter table `order` add constraint FK5ctg5pf4vox82b4orrgg3alc8 foreign key (shipped_from) references location
alter table order_detail add constraint FKplam7wxc4tjbgex0xyk8f0qxo foreign key (order_id) references `order`
...
insert into `order` (city, country, county, street_address, customer, shipped_from) values ('Newmarket', 'Canada', 'Ontario', '75 Corben Place', 1, 2)
insert into `order` (city, country, county, street_address, customer, shipped_from) values ('Paris', 'France', '	Île-de-France', '8 4th Trail', 3, 2)
...
insert into `order` (city, country, county, street_address, customer, shipped_from) values ('Vila Maior', 'Portugal', 'Aveiro', '96072 Fordem Street', 3, 1)
insert into `order` (city, country, county, street_address, customer, shipped_from) values ('Paris', 'France', '	Île-de-France', '8 4th Trail', 3, 2)
insert into `order` (city, country, county, street_address, customer, shipped_from) values ('Edsbyn', 'Sweden', 'Gävleborg', '9 Knutson Place', 1, 3)
...
insert into product (description, name, price, weight, category, supplier) values ('Odyssey', 'Honda', 18227.99, 25674, 2, 4)
